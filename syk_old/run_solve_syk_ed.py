...
Ls = list(range(12,15,2))
seeds = list(range(10))

dir = f'/n/home01/ytan/scratch/deviation_ee/output/solve_syk_ed/'
if not os.path.isdir(dir):
  os.mkdir(dir)
for L in Ls:
  for seed in seeds:
    file_name = os.path.join(dir, f"solve_syk_ed_L={L}_seed={seed}.sh")
    with open (file_name, 'w') as rsh:
      rsh.write(f'''\
#!/bin/bash -l
###### for CPU #####
# #SBATCH -J solve_syk_ed        # Job name
...
# srun --mpi=pmix -n $SLURM_NTASKS python -O /n/home01/ytan/deviation_ee/syk/solve_syk_ed.py -L {L} --seed {seed} --tol 1e-9
...
mamba deactivate
singularity exec --nv /n/holystore01/LABS/yao_lab/Lab/dynamite/sifs/dynamite_latest-cuda.sif python /n/home01/ytan/deviation_ee/syk/solve_syk_ed.py -L {L} --seed {seed} --tol 1e-9
''')
      rsh.close()
