        ...
        for row in reader:
            if not row[0].startswith('#'):
                if int(row[0]) == L:
                    ...
                    break
ees_page = np.array(ees_page).reshape(-1, 1)  # reshape to (len(Ls), 1)
...
with open('ent_entropy.txt', 'w') as f:
    for tol in tols:
        ...
        f.write(f'{"Average entropy for tol=" + str(tol):<{label_width}}: {np.average(ees[tol], axis=1)}\n')
        print(f"\033[92m{'Difference for tol=' + str(tol):<{label_width}}: {np.average(deltas[tol], axis=1)}\033[0m")
        f.write(f"\033[92m{'Difference for tol=' + str(tol):<{label_width}}: {np.average(deltas[tol], axis=1)}\033[0m\n")
        print(f'{"Standard deviation for tol=" + str(tol):<{label_width}}: {np.std(deltas[tol], axis=1)}')
        f.write(f'{"Standard deviation for tol=" + str(tol):<{label_width}}: {np.std(deltas[tol], axis=1)}\n')
# deltas_thermal = {}
# thermal_bound = {}
# for tol in tols:
