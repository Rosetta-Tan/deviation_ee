...
H = Operator.load(join(args.m,'H2_'+str(args.L)+'_'+str(args.H_idx)+'.msc'))

et.begin_event('Solving ground state')
evals, evecs = H.eigsolve(getvecs=True, nev=1, which='smallest', tol=args.tol, subspace=config.subspace)
evec = evecs[0]
