...
def coeff(key):
    ...
    idxs_com_all = list(combinations(idxs_rem, 1))  # 1 index in common, pick all the possible indices
    key_divide = [(combo, tuple(set(key) - set(combo))) for combo in combinations(key, 3)]
    coeff = 0.
    ...
    for idxs_com in idxs_com_all:
        for key1, key2 in key_divide:
            ...
            '''
            Two index tuples have 1 indices in common.
            Insertion rule: Id = chi_1 chi_i
            '''
...
for j in range(n):
    ...
    H2_7uniq = op_sum(coeff(key)*op_product((M[idx] for idx in key)) for key in list(combinations(range(N),6)))
    H2_7uniq.save(join(output_dir,f'H2_7uniq_{L}_{j}.msc'))
    
    # build the 7-element coefficient dictionary
    # maj_terms = {key: 0. for key in list(combinations(range(N),6))}
    # for key in maj_terms.keys():
    #     maj_terms[key] = coeff(key)
    # obj = {}
    # for key, val in maj_terms.items():
    #     obj[str(key)] = val
    # with open (join(output_dir,f'maj_terms_7uniq_{L}_{j}.json'),'w') as f:
