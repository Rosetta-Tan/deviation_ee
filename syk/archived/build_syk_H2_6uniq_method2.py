...
def gen_terms(key):
    ...
    for common_idxs in common_idxs_all:
        ...
        for key1 in key1_all:
            ...
            yield contribution*op_product([M[key1[0]],M[key1[1]],M[common_idxs[0]],M[common_idxs[1]],\
                                           M[common_idxs[0]],M[common_idxs[1]],M[key2[0]],M[key2[1]]])
...
for j in range(n):
    ...
    H2_6uniq_method2 = op_sum([op_sum(gen_terms(key)) for key in combinations(range(N),6)])
    H2_6uniq_method2.save(join(output_dir,f'H2_6uniq_method2_{L}_{j}.msc'))
