...
for tol in tols:
    ...
    for L in Ls:
        ees[tol].append([])
        with open(os.path.join(obs_dir, f'ent_entropy_f=0.33_L={L}_tol={tol}.csv'), 'r') as file:
...
ees_page = []
for L in Ls:
    with open(f'../syk/exact_page_vals_f=0.33.csv', 'r') as file:
        next(file)
        reader = csv.reader(file)
        for row in reader:
...
ax.legend()
fig.savefig('../figures/ent_entropy_f=0.33.pdf', bbox_inches='tight')
